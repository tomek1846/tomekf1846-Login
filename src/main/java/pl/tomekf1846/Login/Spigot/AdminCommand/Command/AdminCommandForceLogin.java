...
public class AdminCommandForceLogin {
    ...
    public static void forceLogin(CommandSender sender, String playerName) {
        ...
        if (player == null || !player.isOnline()) {
            sender.sendMessage(prefix + LanguageManager.getMessage(sender, "messages.admin-commands.player_not_online")
                    .replace("{player}", playerName));
            return;
        }

        if (!PlayerRestrictions.isPlayerBlocked(player)) {
            sender.sendMessage(prefix + LanguageManager.getMessage(sender, "messages.admin-commands.already_logged_in")
        ...
        sender.sendMessage(prefix + LanguageManager.getMessage(sender, "messages.admin-commands.force_login_success")
                .replace("{player}", player.getName()));
    }
}
