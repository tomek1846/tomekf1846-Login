...
public class PlayerCommandChangePassword {
    public static void changePassword(CommandSender sender, String oldPassword, String newPassword) {
        ...
        if (PlayerDataSave.isPlayerPremium(playerName)) {
            sender.sendMessage(getPrefix(player) + LanguageManager.getMessage(player, "messages.player-commands.premium-player"));
            return;
        }
        ...
        if (playerData == null) {
            sender.sendMessage(getPrefix(player) + LanguageManager.getMessage(player, "messages.player-commands.data-not-found"));
            return;
        }

        String storedPassword = playerData.get("Password");
        if (storedPassword == null || !storedPassword.equals(oldPassword)) {
            sender.sendMessage(getPrefix(player) + LanguageManager.getMessage(player, "messages.player-commands.incorrect-old-password"));
            return;
        }
        ...
        if (newPassword.length() < minPasswordLength) {
            sender.sendMessage(getPrefix(player) + LanguageManager.getMessage(player, "messages.player-commands.password-too-short")
                    .replace("{min-length}", String.valueOf(minPasswordLength)));
            return;
        }

        if (isPasswordTooSimple(newPassword)) {
            sender.sendMessage(getPrefix(player) + LanguageManager.getMessage(player, "messages.player-commands.password-too-simple"));
            return;
        }
        ...
        sender.sendMessage(getPrefix(player) + LanguageManager.getMessage(player, "messages.player-commands.success-password-change"));
    }
